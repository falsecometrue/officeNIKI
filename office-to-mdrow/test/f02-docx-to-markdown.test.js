...
test("UT-F02-012: 複数画像をファイル名resourceへ出力し、Markdownから相対参照する", async () => {
  ...
  const imageLinks = markdown.match(/(?:!\[[^\]]*]\(|<img [^>]*src=")UT-F02-012resource\/[^)" >]+/g) || [];
  assert.equal(resources.length, 3);
  assert.ok(imageLinks.length >= 3, `画像リンク数: ${imageLinks.length}`);
  assert.doesNotMatch(markdown, /data:image\//);
});

test("UT-F02-003/004: XML属性値混入、画像サイズ、HTMLリテラルを補正する", async () => {
  const { markdown } = await convertFixture("UT-F02-003_004.docx");

  assert.doesNotMatch(markdown, /preserve/);
  assert.match(markdown, /<img src="UT-F02-003_004resource\/image1\.png" alt="Picture 2" width="201" height="167">/);
  assert.match(markdown, /Format text via header tags, like &lt;h1&gt;/);
  assert.match(markdown, /Tables:\s*&lt;table&gt; tag/);
});
